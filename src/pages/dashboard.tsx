  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        // Prefer batched endpoint (1 round-trip) for faster load.
        let dashboardData: any = {};
        let rentalCostsData: any = {};
        let totalFoodConsumptionData: any = {};
        let totalFoodValueData: any = {};
        let totalAmountSpentData: any = {};
        try {
          const full = await fetchWithCache('/api/dashboard/full', { maxAge: 2 * 60 * 1000 });
          if (full && typeof full.totalAssets === 'number') {
            dashboardData = full;
            rentalCostsData = full.rentalCosts || {
              monthlyTotal: 0, yearlyTotal: 0, monthlyRentalTotal: 0, yearlyRentalTotal: 0,
              monthlyMaintenanceTotal: 0, yearlyMaintenanceTotal: 0
            };
            totalFoodConsumptionData = { totalConsumed: full.totalFoodConsumption ?? full.totalFoodCost ?? 0 };
            totalFoodValueData = { totalValue: full.totalFoodSupplyValue ?? 0 };
            totalAmountSpentData = {
              totalAmountSpent: full.totalAmountSpent ?? 0,
              breakdown: full.amountSpentBreakdown ?? {
                foodConsumption: 0, assetsPurchased: 0, vehicleRentalCosts: 0, vehicleMaintenanceCosts: 0
              }
            };
            logDebug('Dashboard: Loaded from batched /api/dashboard/full');
          } else {
            throw new Error('Invalid full payload');
          }
        } catch (_) {
          dashboardData = await fetchData('/api/dashboard/stats', 2 * 60 * 1000);
          ...
          rentalCostsData = rentalCostsResult.status === 'fulfilled' ? rentalCostsResult.value : {
            monthlyTotal: 0, yearlyTotal: 0, monthlyRentalTotal: 0, yearlyRentalTotal: 0,
            monthlyMaintenanceTotal: 0, yearlyMaintenanceTotal: 0
          };
          totalFoodConsumptionData = foodConsumptionResult.status === 'fulfilled' ? foodConsumptionResult.value : { totalConsumed: 0 };
          totalFoodValueData = foodValueResult.status === 'fulfilled' ? foodValueResult.value : { totalValue: 0 };
          totalAmountSpentData = totalSpentResult.status === 'fulfilled' ? totalSpentResult.value : {
            totalAmountSpent: 0,
            breakdown: { foodConsumption: 0, assetsPurchased: 0, vehicleRentalCosts: 0, vehicleMaintenanceCosts: 0 }
          };
        }

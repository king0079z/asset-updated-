...
/* ── Config (light + dark) ── */
const PRIORITY_CFG = {
  CRITICAL: { label: "Critical", color: "text-red-600 dark:text-red-300",     bg: "bg-red-50 dark:bg-red-950/50",     border: "border-red-200 dark:border-red-800",     dot: "bg-red-500",     barColor: "border-t-red-500",    icon: <ArrowUp className="h-3.5 w-3.5" /> },
  HIGH:     { label: "High",     color: "text-orange-600 dark:text-orange-300",  bg: "bg-orange-50 dark:bg-orange-950/50",  border: "border-orange-200 dark:border-orange-800",  dot: "bg-orange-500",  barColor: "border-t-orange-500",  icon: <ArrowUp className="h-3.5 w-3.5" /> },
  MEDIUM:   { label: "Medium",   color: "text-amber-600 dark:text-amber-300",   bg: "bg-amber-50 dark:bg-amber-950/50",   border: "border-amber-200 dark:border-amber-800",   dot: "bg-amber-500",   barColor: "border-t-amber-500",   icon: <Minus className="h-3.5 w-3.5" /> },
  LOW:      { label: "Low",      color: "text-emerald-600 dark:text-emerald-300", bg: "bg-emerald-50 dark:bg-emerald-950/50", border: "border-emerald-200 dark:border-emerald-800", dot: "bg-emerald-500", barColor: "border-t-emerald-500", icon: <ArrowDown className="h-3.5 w-3.5" /> },
};
const STATUS_CFG = {
  OPEN:        { label: "Open",        color: "text-blue-700 dark:text-blue-300",    bg: "bg-blue-50 dark:bg-blue-950/50",    border: "border-blue-200 dark:border-blue-800",    dot: "bg-blue-500",    icon: <Circle className="h-4 w-4 text-blue-500" /> },
  IN_PROGRESS: { label: "In Progress", color: "text-amber-700 dark:text-amber-300",   bg: "bg-amber-50 dark:bg-amber-950/50",   border: "border-amber-200 dark:border-amber-800",   dot: "bg-amber-500",   icon: <Clock className="h-4 w-4 text-amber-500" /> },
  RESOLVED:    { label: "Resolved",    color: "text-emerald-700 dark:text-emerald-300", bg: "bg-emerald-50 dark:bg-emerald-950/50", border: "border-emerald-200 dark:border-emerald-800", dot: "bg-emerald-500", icon: <CheckCircle2 className="h-4 w-4 text-emerald-500" /> },
  CLOSED:      { label: "Closed",      color: "text-slate-600 dark:text-slate-400",   bg: "bg-slate-100 dark:bg-slate-800/80",  border: "border-slate-200 dark:border-slate-600",   dot: "bg-slate-400",   icon: <XCircle className="h-4 w-4 text-slate-500" /> },
};
const TICKET_TYPE_CFG = {
  ISSUE:      { label: "Issue",     icon: AlertTriangle, color: "text-red-600 dark:text-red-300",    bg: "bg-red-50 dark:bg-red-950/50",    border: "border-red-200 dark:border-red-800",    headerBg: "from-red-600 to-red-700" },
  REQUEST:    { label: "Request",   icon: Inbox,         color: "text-blue-600 dark:text-blue-300",   bg: "bg-blue-50 dark:bg-blue-950/50",   border: "border-blue-200 dark:border-blue-800",   headerBg: "from-blue-600 to-blue-700" },
  INQUIRY:    { label: "Inquiry",   icon: HelpCircle,    color: "text-purple-600 dark:text-purple-300", bg: "bg-purple-50 dark:bg-purple-950/50", border: "border-purple-200 dark:border-purple-800", headerBg: "from-purple-600 to-purple-700" },
  MANAGEMENT: { label: "Mgmt",      icon: FileText,      color: "text-slate-600 dark:text-slate-300",  bg: "bg-slate-50 dark:bg-slate-800/80",  border: "border-slate-200 dark:border-slate-600",  headerBg: "from-slate-700 to-slate-800" },
};
...
/* ── Skeleton ── */
function Sk({ className = "" }) { return <div className={`animate-pulse rounded-xl bg-slate-100 dark:bg-slate-800 ${className}`} />; }
...
function TicketDetailsContent() {
  ...
  if (!ticket) {
    return (
      <div className="flex flex-col items-center gap-4 rounded-2xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-900 py-20 text-center">
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-slate-100 dark:bg-slate-800">
          <AlertCircle className="h-8 w-8 text-slate-400 dark:text-slate-500" />
        </div>
        <div><p className="font-bold text-slate-900 dark:text-slate-100">Ticket not found</p><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">This ticket doesn't exist or you don't have access.</p></div>
        <Link href="/tickets" className="inline-flex items-center gap-1.5 rounded-xl bg-slate-900 dark:bg-slate-100 px-4 py-2 text-sm font-semibold text-white dark:text-slate-900 hover:bg-slate-800 dark:hover:bg-slate-200 transition-colors">
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-start gap-4">
          <Link href="/tickets"
            className="mt-1 inline-flex items-center gap-1.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2 text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors shrink-0">
            <ArrowLeft className="h-4 w-4" /> Back
          </Link>
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-1">
              <h1 className="text-2xl font-black tracking-tight text-slate-900 dark:text-slate-100 line-clamp-2">{ticket.title}</h1>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <span className="font-mono text-sm font-bold text-slate-400 dark:text-slate-500">{formatTicketId(ticket.displayId, ticket.id)}</span>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button onClick={() => { if (typeof id === "string") silentRefresh(id); }}
            className="rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 p-2 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
      ...
      {availableActions.length > 0 && (
        <div className="flex flex-wrap gap-2 rounded-2xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-900 p-4">
          <span className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 mr-2">
      ...
      {assignStartOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/50 dark:bg-black/60 backdrop-blur-sm" onClick={() => setAssignStartOpen(false)} />
          <div className="relative w-full max-w-md rounded-2xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 shadow-2xl overflow-hidden mx-4">
            <div className={`flex items-center gap-3 border-b border-slate-200 dark:border-slate-700 px-5 py-4 ${tc.bg} ${tc.border}`}>
              <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border bg-white dark:bg-slate-800 ${tc.border}`}>
            ...
            <div className="p-5 space-y-4">
              <p className="text-sm text-slate-600 dark:text-slate-300 line-clamp-2">&quot;{ticket.title}&quot;</p>
              <div>
          ...
          {STEPS.map((step, i) => {
            ...
            return (
              <div key={step} className="flex flex-1 flex-col items-center gap-1.5">
                <div className={`relative h-2.5 w-full overflow-hidden rounded-full transition-all duration-700 ${done ? c.dot : "bg-slate-100 dark:bg-slate-700"}`}>
                  {active && <div className="absolute inset-0 animate-pulse opacity-60 bg-white dark:bg-slate-300" />}
                </div>
                <span className={`text-center text-[10px] font-bold leading-tight ${done ? c.color : "text-slate-300 dark:text-slate-500"}`}>{c.label}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-5">
          {/* Tabs */}
          <div className="flex gap-1 rounded-2xl border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/80 p-1">
            {[
              ...
              return (
                <button key={tab.key} onClick={() => setActiveTab(tab.key as any)}
                  className={`flex flex-1 items-center justify-center gap-1.5 rounded-xl py-2.5 text-sm font-semibold transition-all ${activeTab === tab.key ? "bg-white dark:bg-slate-800 shadow text-slate-900 dark:text-slate-100" : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200"}`}>
          ...
          {activeTab === "details" && (
            <div className="space-y-4">
              {/* Description */}
              <div className="rounded-2xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
                <p className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Description</p>
                <div className="rounded-xl bg-slate-50 dark:bg-slate-800/80 p-4 text-sm leading-relaxed text-slate-700 dark:text-slate-200 whitespace-pre-wrap">
                  {ticket.description}
                </div>
              </div>
              ...
              {(ticket.location || ticket.contactDetails || ticket.category || ticket.subcategory) && (
                <div className="rounded-2xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
                  <p className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Request Details</p>
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    {ticket.category && (
                      <div className="flex items-start gap-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/80 p-3">
                        <Tag className="mt-0.5 h-4 w-4 shrink-0 text-slate-400 dark:text-slate-500" />
                        <div><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Category</p><p className="text-sm font-semibold text-slate-700 dark:text-slate-200">{ticket.category.replace("_", " ")}</p></div>
                      </div>
                    )}
                    {ticket.subcategory && (
                      <div className="flex items-start gap-2.5 rounded-xl bg-blue-50 dark:bg-blue-950/50 border border-blue-100 dark:border-blue-800 p-3">
                        <Settings className="mt-0.5 h-4 w-4 shrink-0 text-blue-400" />
                        <div><p className="text-[10px] font-bold uppercase tracking-wider text-blue-400 dark:text-blue-400">Service Type</p><p className="text-sm font-semibold text-blue-700 dark:text-blue-300">{ticket.subcategory}</p></div>
                      </div>
                    )}
                    {ticket.location && (
                      <div className="flex items-start gap-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/80 p-3">
                        <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-slate-400 dark:text-slate-500" />
                        <div><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Location</p><p className="text-sm font-semibold text-slate-700 dark:text-slate-200">{ticket.location}</p></div>
                      </div>
                    )}
                    {ticket.contactDetails && (
                      <div className="flex items-start gap-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/80 p-3">
                        <Phone className="mt-0.5 h-4 w-4 shrink-0 text-slate-400 dark:text-slate-500" />
                        <div><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Contact</p><p className="text-sm font-semibold text-slate-700 dark:text-slate-200">{ticket.contactDetails}</p></div>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {ticket.asset && (
                <div className="rounded-2xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
                  <p className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Related Asset</p>
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-800">
                      <Package className="h-5 w-5 text-slate-500 dark:text-slate-400" />
                    </div>
                    <div>
                      <p className="font-semibold text-slate-900 dark:text-slate-100">{ticket.asset.name}</p>
                      <p className="font-mono text-xs text-slate-400 dark:text-slate-500">{ticket.asset.assetId}</p>
                    </div>
                    <Link href={`/assets/${ticket.asset.id}`}
                      className="ml-auto inline-flex items-center gap-1 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-800 px-3 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
          ...
          {/* History tab */}
          {activeTab === "history" && (
            <div className="rounded-2xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-900">
          ...
          {activeTab === "ai" && (
            <div className="rounded-2xl border border-violet-200 dark:border-violet-800 bg-gradient-to-br from-violet-50 to-white dark:from-violet-950/40 dark:to-slate-900 overflow-hidden">
              <div className="flex items-center justify-between border-b border-violet-100 dark:border-violet-800 bg-gradient-to-r from-violet-900 to-slate-900 dark:from-violet-950 dark:to-slate-950 px-5 py-4">
                <div className="flex items-center gap-3">
              ...
              <div className="p-5 space-y-5">
                {aiLoading ? (
                  <div className="flex items-center justify-center gap-3 py-8">
                    ...
                    <p className="text-sm text-violet-600 dark:text-violet-400 font-medium">Analysing ticket…</p>
                  </div>
                ) : aiInsights ? (
                  <>
                    {/* SLA Status */}
                    <div className={`rounded-2xl border p-4 ${aiInsights.sla?.breached ? "border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/50" : "border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/50"}`}>
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <Shield className={`h-4 w-4 ${aiInsights.sla?.breached ? "text-red-600" : "text-emerald-600"}`} />
                          <p className={`text-sm font-bold ${aiInsights.sla?.breached ? "text-red-700 dark:text-red-300" : "text-emerald-700 dark:text-emerald-300"}`}>
                            SLA: {aiInsights.sla?.breached ? "⚠ Breached" : "✓ On Track"} — Target: {aiInsights.sla?.target}
                          </p>
                        </div>
                        <span className={`text-sm font-black ${aiInsights.sla?.breached ? "text-red-700 dark:text-red-300" : "text-emerald-700 dark:text-emerald-300"}`}>
                          {aiInsights.sla?.progressPct}%
                        </span>
                      </div>
                      <div className="h-2.5 w-full rounded-full bg-white/60 dark:bg-slate-700 overflow-hidden">
                        <div
                    ...
                    {aiInsights.sentiment && (
                      <div className="flex items-center gap-3 rounded-2xl bg-white dark:bg-slate-800/80 border border-slate-100 dark:border-slate-700 px-4 py-3">
                        <span className="text-2xl">{aiInsights.sentiment.emoji}</span>
                        <div>
                          <p className="text-sm font-bold text-slate-800 dark:text-slate-200">Requester Sentiment: {aiInsights.sentiment.sentiment}</p>
                          <p className="text-xs text-slate-500 dark:text-slate-400">Detected from ticket description</p>
                        </div>
                      </div>
                    )}
                    ...
                    {aiInsights.insights?.length > 0 && (
                      <div className="space-y-2">
                        <p className="text-xs font-bold uppercase tracking-wider text-violet-600 dark:text-violet-400">AI Insights</p>
                        {aiInsights.insights.map((ins: string, i: number) => (
                          <div key={i} className="flex items-start gap-2.5 rounded-xl bg-white dark:bg-slate-800/80 border border-violet-100 dark:border-violet-800 px-3.5 py-2.5">
                            <Sparkles className="mt-0.5 h-3.5 w-3.5 shrink-0 text-violet-500" />
                            <p className="text-sm text-slate-700 dark:text-slate-200">{ins}</p>
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    {aiInsights.suggestedAssignee && (
                      <div className="rounded-2xl border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-950/50 p-4">
                        <div className="flex items-center gap-2 mb-2">
                          <Target className="h-4 w-4 text-blue-600 dark:text-blue-400" />
                          <p className="text-sm font-bold text-blue-700 dark:text-blue-300">AI Suggested Assignee</p>
                        </div>
                        <div className="flex items-center gap-3">
                          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-700 dark:bg-blue-600 text-sm font-bold text-white">
                            {aiInsights.suggestedAssignee.name[0]}
                          </div>
                          <div>
                            <p className="font-semibold text-slate-900 dark:text-slate-100">{aiInsights.suggestedAssignee.name}</p>
                            <p className="text-xs text-slate-500 dark:text-slate-400">{aiInsights.suggestedAssignee.email} · {aiInsights.suggestedAssignee.activeTickets} active tickets</p>
                            <p className="text-xs text-blue-600 dark:text-blue-400 font-medium">{aiInsights.suggestedAssignee.reason}</p>
                          </div>
                          <button
                            onClick={() => assignTicket(aiInsights.suggestedAssignee.id)}
                            className="ml-auto rounded-xl bg-blue-600 dark:bg-blue-500 px-3 py-1.5 text-xs font-bold text-white hover:bg-blue-700 dark:hover:bg-blue-600 transition-colors">
                            Assign Now
                          </button>
                        </div>
                      </div>
                    ...
                    {aiInsights.benchmark && (
                      <div className="flex items-center justify-between rounded-2xl bg-white dark:bg-slate-800/80 border border-slate-100 dark:border-slate-700 px-4 py-3">
                        <div className="flex items-center gap-2">
                          <TrendingUp className="h-4 w-4 text-slate-400 dark:text-slate-500" />
                          <p className="text-sm text-slate-600 dark:text-slate-300">Similar ticket avg. resolution</p>
                        </div>
                        <div className="text-right">
                          <p className="text-base font-black text-slate-800 dark:text-slate-100">{aiInsights.benchmark.avgResolutionFormatted}</p>
                          <p className="text-[10px] text-slate-400 dark:text-slate-500">{aiInsights.benchmark.sampleSize} tickets</p>
                        </div>
                      </div>
                    )}
                  </>
                  ...
                  <div className="flex flex-col items-center gap-2 py-8 text-center">
                    <Brain className="h-10 w-10 text-violet-200 dark:text-violet-700" />
          ...
          <div className="rounded-2xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
            ...
            <textarea
              ...
              className="w-full resize-none rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-800 px-3.5 py-2.5 text-sm text-slate-800 dark:text-slate-200 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-slate-400 dark:focus:border-slate-500 focus:bg-white dark:focus:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-200 dark:focus:ring-slate-600 transition-colors"
            />
            <div className="mt-3 flex items-center justify-between gap-3">
              <p className="text-xs text-slate-400 dark:text-slate-500">
                {status !== ticket.status && <span className="font-semibold text-amber-600 dark:text-amber-400">Status: {ticket.status} → {status}</span>}
                {priority !== ticket.priority && <span className="ml-2 font-semibold text-amber-600 dark:text-amber-400">Priority: {ticket.priority} → {priority}</span>}
              </p>
              <button onClick={updateTicket}
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-slate-900 dark:bg-slate-100 px-5 py-2.5 text-sm font-semibold text-white dark:text-slate-900 hover:bg-slate-800 dark:hover:bg-slate-200 disabled:opacity-40 transition-colors">
        ...
        <div className="space-y-4">
          {/* Status */}
          <div className={`rounded-2xl border-t-4 border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-900 p-5 ${pc.barColor}`}>
            <p className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Status</p>
            <div className="mb-3 flex items-center gap-2">{sc.icon}<SBadge s={ticket.status} /></div>
            <select value={status} onChange={e => setStatus(e.target.value as TicketStatus)}
              className="w-full appearance-none rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-800 px-3 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-200 focus:border-slate-400 dark:focus:border-slate-500 focus:bg-white dark:focus:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-200 dark:focus:ring-slate-600 transition-colors">
          ...
          {/* Priority */}
          <div className="rounded-2xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
            <p className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Priority</p>
            ...
            <div className="grid grid-cols-2 gap-2">
              {Object.entries(PRIORITY_CFG).map(([key, cfg]) => (
                <button key={key} onClick={() => setPriority(key as TicketPriority)}
                  className={`flex items-center gap-1.5 rounded-xl border-2 px-2.5 py-2 text-xs font-semibold transition-all ${priority === key ? `${cfg.bg} ${cfg.border} ${cfg.color}` : "border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:border-slate-200 dark:hover:border-slate-600"}`}>
                  {cfg.icon}{cfg.label}
                </button>
              ))}
            </div>
          ...
          {/* Assign */}
          <div className="rounded-2xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
            <p className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Assigned To</p>
            {ticket.assignedTo ? (
              <div className="mb-3 flex items-center gap-2.5 rounded-xl bg-violet-50 dark:bg-violet-950/50 p-3 border border-violet-100 dark:border-violet-800">
                <div className="flex h-8 w-8 items-center justify-center rounded-full bg-violet-700 dark:bg-violet-600 text-xs font-bold text-white">
                  {ticket.assignedTo.email[0].toUpperCase()}
                </div>
                <div>
                  <p className="text-xs font-bold text-violet-700 dark:text-violet-300">{ticket.assignedTo.email.split("@")[0]}</p>
                  <p className="text-[10px] text-violet-400 dark:text-violet-500">{ticket.assignedTo.email}</p>
                </div>
              </div>
            ) : (
              <p className="mb-3 rounded-xl bg-slate-50 dark:bg-slate-800 px-3 py-2 text-xs text-slate-400 dark:text-slate-500">Not assigned yet</p>
            )}
            <div className="relative">
              <select
                ...
                disabled={isUpdating}
                className="w-full appearance-none rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-800 px-3 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-200 focus:border-slate-400 dark:focus:border-slate-500 focus:bg-white dark:focus:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-200 dark:focus:ring-slate-600 transition-colors disabled:opacity-50 cursor-pointer">
              ...
              {isUpdating && <Loader2 className="absolute right-2.5 top-1/2 h-4 w-4 -translate-y-1/2 animate-spin text-slate-400 dark:text-slate-500" />}
            </div>
          </div>
          ...
          {(ticket.user || ticket.requesterName) && (
            <div className="rounded-2xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
              <p className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Submitted By</p>
              <div className="flex items-center gap-3 mb-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-slate-600 to-slate-800 dark:from-slate-500 dark:to-slate-700 text-sm font-bold text-white">
                  {(ticket.user?.email || ticket.requesterName || "U")[0].toUpperCase()}
                </div>
                <div>
                  <p className="font-semibold text-slate-900 dark:text-slate-100">{ticket.requesterName || ticket.user?.email?.split("@")[0] || "Unknown User"}</p>
          ...
          {/* Dates & Meta */}
          <div className="rounded-2xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
            <p className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Timeline</p>
            <div className="space-y-3 text-sm text-slate-600 dark:text-slate-300">
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-slate-700 dark:text-slate-200">{safeFormatDate(ticket.createdAt)}</p>
                </div>
              </div>
              <div className="flex items-start gap-2.5">
                <div className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-slate-700 dark:text-slate-200">{timeAgo(ticket.updatedAt)}</p>
                </div>
              </div>
            </div>
          </div>

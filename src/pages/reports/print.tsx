...
export default function PrintReportPage() {
  ...
  const formatReportContent = (dataOverride?: any) => {
    ...
    return `
      <style>
        .report-page { font-family: 'Segoe UI', system-ui, -apple-system, sans-serif; color: #111827; line-height: 1.5; }
        .report-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 2rem; padding-bottom: 1.25rem; border-bottom: 2px solid #e5e7eb; }
        .report-header h1 { margin: 0; font-size: 1.75rem; font-weight: 700; letter-spacing: -0.02em; color: #111827; }
        .report-meta { margin-top: 0.5rem; font-size: 0.875rem; color: #6b7280; }
        .report-brand { text-align: right; font-weight: 600; font-size: 1rem; color: #374151; }
        .report-id { font-size: 0.75rem; color: #9ca3af; margin-top: 0.25rem; }
        .report-body { margin: 2rem 0; }
        .report-footer { margin-top: 2.5rem; padding-top: 1rem; border-top: 1px solid #e5e7eb; font-size: 0.75rem; color: #9ca3af; text-align: center; }
        .report-footer p { margin: 0.25rem 0; }
        @media print { .report-header, .report-body, .report-footer { page-break-inside: avoid; } }
      </style>
      <div class="report-page p-8 max-w-full">
        <header class="report-header">
          <div>
            <h1>${reportTitle}</h1>
            <p class="report-meta">Generated on ${dateStr}</p>
            <p class="report-meta">Generated by ${user?.email || 'System'}</p>
          </div>
        ...
        <div class="report-body">${reportContentHtml}</div>
        <footer class="report-footer">
          <p>This report was automatically generated by the Enterprise Asset Management System.</p>
          <p>© ${new Date().getFullYear()} Enterprise Asset Management. Confidential.</p>
        </footer>
      </div>
    `;
  };
            ...
            fetch("/api/reports/consumption-summary")
              ...
              .then(async (data) => {
                ...
                try {
                  ...
                  toast({ title: "Error printing report", description: "Print could not be started.", variant: "destructive" });
                }
                setIsPrinting(false);
              })
              .catch(error => {
              ...
              <Button variant="outline" className="w-full mt-2" onClick={(e) => {
                ...
                fetch("/api/reports/consumption-summary")
                  ...
                  .then(async (data) => {
                    setReportData(data);
                    const bodyHtml = `<div id="consumption-report">${renderConsumptionSummaryReport(data)}</div>`;
                    const fullHtml = `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>${t("consumption_summary")}</title><style>body{font-family:system-ui,sans-serif;margin:1rem;} table{width:100%;border-collapse:collapse;} th,td{border:1px solid #e5e7eb;padding:8px;} th{background:#f9fafb;}</style></head><body>${bodyHtml}</body></html>`;
                    try {
                      await printContentWithIframe(fullHtml, t("consumption_summary"));
                      toast({ title: t("report_generated"), description: format(new Date(), "PPP p") });
                    } catch (err) {

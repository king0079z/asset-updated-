...
// Only accept small JSON — no blobs here
export const config = {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const {
    ...
    assignedToId,
    ticketId,
    signatureDataUrl, // Small canvas PNG — kept here for history preview
    signedAt,
  } = req.body ?? {};
  ...
  try { roleData = await getUserRoleData(user.id); } catch { /* proceed */ }
  const orgId = roleData?.organizationId;

  const assetWhere: any = { id: assetId };
  if (orgId) assetWhere.OR = [{ organizationId: orgId }, { organizationId: null }];
      ...
      await tx.assetHistory.create({
        data: {
          ...
          details: {
            ...
            assignedToEmail: assignedToEmail?.trim() || null,
            assignedToId:    assignedToId   || null,
            signatureDataUrl,            // canvas PNG (small, fits in JSON)
            documentId:      null,       // patched later when PDF is uploaded

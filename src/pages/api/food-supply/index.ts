...
const FOOD_SUPPLY_LIST_CACHE = new Map<string, { data: any[]; ts: number }>();
const FOOD_SUPPLY_CACHE_TTL = 2 * 60 * 1000; // 2 min
const FOOD_SUPPLY_TAKE = 1000;
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  res.setHeader('Cache-Control', 'private, max-age=60, stale-while-revalidate=30');
    ...
    const cacheKey = `${user.id}:${kitchenId ?? ''}:${expiringSoon ?? ''}:${lowStock ?? ''}:${category ?? ''}`;
    const cached = FOOD_SUPPLY_LIST_CACHE.get(cacheKey);
    if (cached && Date.now() - cached.ts < FOOD_SUPPLY_CACHE_TTL && !barcode) {
      res.setHeader('Cache-Control', 'private, max-age=120, stale-while-revalidate=60');
      if (expiringSoon === 'true' || lowStock === 'true') {
        return res.status(200).json({ items: cached.data });
      }
    ...
    const foodSupplies = await prisma.foodSupply.findMany({
      ...
      take: FOOD_SUPPLY_TAKE,
    });

    if (!barcode) FOOD_SUPPLY_LIST_CACHE.set(cacheKey, { data: foodSupplies, ts: Date.now() });

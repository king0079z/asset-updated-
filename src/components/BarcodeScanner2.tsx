      ...
      <DialogPrimitive.Root open={isOpen} onOpenChange={o => !o && close()}>
        <DialogPrimitive.Portal>
          ...
          <div className="relative w-full h-full rounded-[28px] overflow-hidden bg-[#09090f]"
            ...
            {/* ══ TOP BAR — balanced 3-column: left (back/icon), center (title), right (close) ══ */}
            <div className="absolute top-0 inset-x-0 z-50 flex items-center justify-between gap-3 px-4 py-4 min-h-[52px]">
              ...
              <h1 className="flex-1 text-center text-white/90 text-[15px] font-semibold tracking-tight truncate">
                {headerTitle[view]}
              </h1>
            ...
            {(view === 'camera' || view === 'manual') && (
              <div className="absolute inset-0">
                ...
                {/* viewfinder overlay — single hole-punch + one clear white frame, no decorative corners */}
                {view === 'camera' && (
                  <div className="absolute inset-0 pointer-events-none flex flex-col items-center justify-center" style={{ zIndex: 2 }}>
                    {/* Semi-transparent dim outside viewfinder (hole-punch) */}
                    <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none" aria-hidden>
                      <defs>
                        <mask id="scan-mask">
                          <rect width="100%" height="100%" fill="white" />
                          <rect x="50%" y="50%" width="280" height="220"
                    ...
                    {/* Single high-contrast viewfinder frame (centered with flex) */}
                    <div
                      className="relative flex-shrink-0 rounded-2xl border-2 border-white shadow-[0_0_0_2px_rgba(255,255,255,0.3),0_0_20px_rgba(255,255,255,0.15)]"
                      style={{ width: 280, height: 220 }}
                    >
                      {/* Laser line — moves up/down for active scanning feedback */}
                      <div
                    ...
                    {/* bottom fade */}
                    <div className="absolute bottom-0 inset-x-0 h-2/5 bg-gradient-to-t from-[#09090f] via-[#09090f]/60 to-transparent pointer-events-none" />
                  </div>
                )}
                ...
                {/* camera bottom hint — consistent center alignment and spacing */}
                {view === 'camera' && !camLoading && !camError && (
                  <div className="absolute bottom-0 inset-x-0 z-10 pb-8 px-6 flex flex-col items-center justify-center text-center gap-4 max-w-[320px] mx-auto">
                    ...
                    <button onClick={() => { stopCam(); setView('manual'); }}
                      className="w-full min-h-[44px] flex items-center justify-center gap-2 text-foreground font-medium text-sm transition-colors py-3 px-4 rounded-xl border border-white/20 bg-white/10 hover:bg-white/15 touch-manipulation">

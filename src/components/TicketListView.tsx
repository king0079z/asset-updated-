...
const TICKET_TYPE_LABELS: Record<string, { label: string; bg: string; text: string }> = {
  ISSUE:      { label: "Issue",     bg: "bg-red-50 dark:bg-red-950/50",     text: "text-red-700 dark:text-red-300" },
  REQUEST:    { label: "Request",   bg: "bg-blue-50 dark:bg-blue-950/50",    text: "text-blue-700 dark:text-blue-300" },
  INQUIRY:    { label: "Inquiry",   bg: "bg-purple-50 dark:bg-purple-950/50",  text: "text-purple-700 dark:text-purple-300" },
  MANAGEMENT: { label: "Mgmt",       bg: "bg-slate-100 dark:bg-slate-800",  text: "text-slate-700 dark:text-slate-300" },
};
  ...
  if (isLoading) {
    return (
      <div className="space-y-4">
        {[1, 2, 3].map((i) => (
          <div key={i} className="flex items-center justify-between p-4 border border-border dark:border-slate-700 rounded-lg bg-card dark:bg-slate-900/50">
  ...
  return (
    <div className="space-y-2">
      ...
      {tickets.map((ticket) => (
        <div 
          key={ticket.id} 
          className="grid grid-cols-1 md:grid-cols-12 gap-4 p-4 border border-border dark:border-slate-700 rounded-lg bg-card dark:bg-slate-900/50 hover:bg-accent/10 dark:hover:bg-slate-800/80 transition-colors"
        >
